...
int main() {
    ...
    KafkaProducer producer("localhost:9092", "orders_status");

    // --------------------------------------------Основные обработчики-------------------------------------------------
    OrdersToPayConsumer consumer;
    consumer.listen_orders_to_pay_topic(db);
    start_outbox(db, producer);
    // -----------------------------------------------------------------------------------------------------------------
    ...
    server.Get("/account/:user_id/balance", [&db, &set_cors_headers, &controller](const httplib::Request& request, httplib::Response &res) {
        set_cors_headers(res);
