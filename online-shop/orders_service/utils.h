...
inline void start_outbox_processor(Database& db, KafkaProducer& producer) {
    std::thread([&db, &producer]() {
        while (true) {
            try {
                std::string select_query = "SELECT id, user_id, description, amount FROM orders_storage.outbox WHERE status = $1";
